...
class SettingKnowHowViewController: UIViewController,UIImagePickerControllerDelegate,UINavigationControllerDelegate  {
    ...
    @IBOutlet weak var requiredMaterial: UITextView!
    ...
    private func setUpView(){
        ...
        requiredMaterial.layer.borderColor = UIColor.black.cgColor
        requiredMaterial.layer.borderWidth = 0.1
        requiredMaterial.layer.cornerRadius = 5
        requiredMaterial.layer.masksToBounds = false
        
        if let requiredMate = self.myPostData.requiredMaterial{
            self.requiredMaterial.text = requiredMate
        }else{
            self.requiredMaterial.text = ""
        }
    ...
    @IBAction func changedKnowHow(_ sender: UIBarButtonItem) {
        ...
        }else if requiredTimeArray[requiredSegmented.selectedSegmentIndex] == "1時間以上"{
        ...
        if let user = Auth.auth().currentUser{
            ...
            let postdic = ["title":titleTextField.text!,"name":name!,"detail":detailTextView.text!,"rateId":rateId,"categoryId":categoryId,"time":String(time),"requiredTimeId":requiredTimeId,"requiredMaterialId":self.requiredMaterial.text!,"userId":userId] as [String : Any]
